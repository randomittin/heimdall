...
### INV-6 — the public surface holds NO dispatch capability and does NO cred READ; it may WRITE-FORWARD the caller's OWN cred
...
**REFINEMENT (clarified 2026-07-04, tenant-onboarding).** The intent of INV-6 is precise: **the public surface must never READ a credential and never DISPATCH.** The last-mile onboarding route `POST /team/cred` (`register_team_cred`, cp_publicsurface.py) WRITE-FORWARDS the CALLER'S OWN model cred into EXACTLY the caller's server-derived `team_id` partition (Secret Manager in cloud) via `cp_team_creds.put_team_cred` and STOPS — it is the ONLY cred symbol reachable in this module's CODE. A write-forward of the caller's own secret is **permitted** and does NOT breach INV-6, because it (i) is keyed strictly on `registered_team(haid)` (INV-1 — a body `team_id` is ignored, so it can never reach ANOTHER team's partition, INV-2), (ii) calls ONLY the WRITE (`put_team_cred`), never a cred-READ (`get_team_cred`/`env_for_team`, which live only on the gated worker), (iii) never echoes/logs/returns the secret (INV-4 — the response is `{stored, kind, env_name, team_id}` only), and (iv) never dispatches. INV-6's blast-radius property is therefore intact: a popped public surface holds nothing at rest, cannot READ any cred (its own or another tenant's), and cannot exec. The earlier whole-file `grep` for cred/dispatch symbols was TOO COARSE — it flagged the module's PROSE (comments explaining WHY the write-forward is safe), not code. The refined check forbids cred-READ + dispatch symbols in CODE ONLY (comments/docstrings stripped) while ALLOWING `put_team_cred`.
- **Residual (accepted).** The public process momentarily handles the caller's OWN secret IN TRANSIT to Secret Manager on `POST /team/cred`. This is acceptable: it is the caller's own secret (not another tenant's), the flow is WRITE-ONLY (no read-back route on this surface), and the secret is never persisted in the public process nor placed in any log/response. The residual is strictly bounded to the caller's own partition and does not expand the surface's capability to read or dispatch.
- Testable: on the public surface the `POST /dispatch` code path reaches `create_job` and returns, and NEVER calls `select_maintainer_auth` / `chosen.dispatch`. The refined CODE-ONLY scan (tokenize strips comments + docstrings) asserts cp_publicsurface.py references NO cred-READ symbol (`get_team_cred`, `env_for_team`) and NO dispatch symbol (`select_maintainer_auth`, `.dispatch`, `mint_token_for_team`); the write-forward `put_team_cred` is ALLOWED. The check is falsifiable: injecting a `get_team_cred(...)` call into the module's CODE flips the assertion RED, while a comment mentioning the symbol does NOT (see `test/heimdall-cp-authz-gate.test.sh`, the INV-6 refined block). The gated-vs-public route parity check (`check-public-surface.sh`) still passes.
