...
if [ -n "${LIVE_IMAGE}" ] && printf '%s' "${LIVE_IMAGE}" | grep -qF ":${HEAD_SHA}"; then
  ...
  run gcloud run deploy "${GATED_SERVICE}" \
    ...
    --set-env-vars="HEIMDALL_STATE_BACKEND=firestore,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},HEIMDALL_CP_SERVER_HAID=cp-server,HEIMDALL_JOB_RUNNER=cloudrun-job,HEIMDALL_CP_JOB_NAME=heimdall-maintainer-job,HEIMDALL_RR_TENANT_AUTHZ=1,HEIMDALL_TEAM_CRED_STORE=secretmanager,HEIMDALL_FIX_WITH_CLAUDE=1" \
