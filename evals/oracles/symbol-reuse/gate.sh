#!/usr/bin/env bash
# gate.sh — symbol-reuse differential gate, the registry gate_command driver.
#
# The registry (evals/oracles/registry.json) resolves this domain to:
#     evals/oracles/symbol-reuse/gate.sh --differential --seeds 200
# (bin/oracle-select symbol-reuse prints exactly that). The LIVE seeded arm: for each of N
# seeds the INDEPENDENT reference generator (reference.generate_stream) produces a deterministic
# symbol-declaration stream; the SAME stream is folded by BOTH the impl (redum.detect_symbol_reuse,
# via synthesized source) and the independent reference; both are normalized to blocked/advised/ok
# and DIFFED. PASS iff impl == reference on every seed.
#
...
usage() {
  cat <<'EOF'
symbol-reuse gate.sh — seeded impl-vs-reference differential arm

Usage:
  gate.sh --differential [--seeds N] [--start S] [--records R]
  gate.sh --help

  --differential   Run the seeded impl-vs-reference symbol-reuse differential (the only mode).
                   For each seed: generate one symbol-declaration stream, fold it through the
                   impl (redum, via synthesized source) AND the independent reference, normalize
                   to blocked/advised/ok, diff.
  --seeds N        Seeds to sweep (default 200; the registered floor).
  --start S        First seed (default 1).
  --records R      Symbol declarations per generated stream (default 24).
