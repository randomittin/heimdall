#!/usr/bin/env bash
# run.sh — symbol-reuse differential oracle, STRUCTURED CONTRACT entry point
...
#   kind "differential" (golden): synthesize source from the seeded stream, fold it through the
#     impl (redum.detect_symbol_reuse) AND the INDEPENDENT reference (reference.fold), normalize
#     to blocked/advised/ok, diff. status=pass iff they agree.
...
case "$STATUS" in
  pass)
    write_pass "Impl symbol-reuse classification equals the independent reference on the blocked/advised/ok partition — SR-A..SR-H hold."
  ...
  fail)
    ...
    write_fail "$STEP" "$EXP" "$ACT" \
      "Symbol-reuse classification diverges from the independent reference. Re-derive from INVARIANTS.md: ignored-path symbols DROPPED (SR-A); opt-out symbols ALLOWED (SR-B); exact function/type duplication BLOCKED (SR-C/SR-D); structural-type/same-name/const ADVISED (SR-E..SR-G); unique symbols OK (SR-H). The blocked/advised/ok buckets must match row-for-row."
