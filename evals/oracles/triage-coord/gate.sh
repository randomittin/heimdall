#!/usr/bin/env bash
# gate.sh — triage-coord differential gate, the registry gate_command driver.
#
# The registry (evals/oracles/registry.json) resolves this domain to:
#     evals/oracles/triage-coord/gate.sh --differential --seeds 200
# (bin/oracle-select triage-coord prints exactly that). The LIVE seeded arm: for each of N seeds
...
usage() {
  cat <<'EOF'
triage-coord gate.sh — seeded impl-vs-reference differential arm
