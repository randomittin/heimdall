#!/usr/bin/env bash
# run.sh — team-copilot differential oracle, STRUCTURED CONTRACT entry point (REPORT-CONTRACT.md
# spec 2A). bin/falsify + the orchestrator consume the TYPED report.json this writes — they NEVER
# parse this script's stdout.
#
# Grade ONE fixture through the single source of diff-truth (differential.py):
#   kind "differential" (golden): fold the SAME seeded streams through the impl
#     (issue_claim.simulate_claim_stream) AND the INDEPENDENT reference (reference.py), normalize
#     to the served-claim partition, diff. status=pass iff they agree.
...
#     status=fail -> KILLED.
#
# Report schema (spec H-1): gate_id, status, first_divergence{file,step,expected,actual}|null,
# metrics, fix_hint, haid, wave, ts.
#
...
case "$STATUS" in
  pass)
    write_pass "Impl claim-arbitration fold equals the independent reference on the served-claim partition — one-owner-per-issue-per-team, TTL-reap, and team isolation hold."
  ...
  fail)
    ...
    write_fail "$STEP" "$EXP" "$ACT" \
      "Served-claim sequence diverges from the independent reference. Re-derive from INVARIANTS.md: one grant per (team,issue) at a time (double-pick-race); a pick in another team is NEVER blocked by this team's claim (cross-team-read); a TTL-expired claim is reaped so a later pick is GRANTED (stale-claim-not-reaped)."
