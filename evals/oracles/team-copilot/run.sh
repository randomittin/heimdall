...
#   kind "differential" (golden): fold the SAME stream through the REAL redum team lens AND the
#     INDEPENDENT reference (reference.py), normalize to the team-candidate roster, diff.
...
case "$STATUS" in
  pass)
    write_pass "Redum team-lens roster equals the independent reference on the team-candidate partition — RT-ISO/RT-CLASS/RT-DROP/RT-LEAK hold."
  ...
  fail)
    ...
    write_fail "$STEP" "$EXP" "$ACT" \
      "Team-candidate roster diverges from the independent reference. Re-derive from INVARIANTS.md: a cross-team record NEVER surfaces (RT-ISO); an active claim => teammate-in-flight, a reaped claim with a surviving checkpoint => adoptable-dropped, pct>=100/done => completed (RT-CLASS); every relevant teammate row is present exactly once (RT-DROP); a candidate carries ONLY scrubbed roster fields, never goal prose (RT-LEAK)."
