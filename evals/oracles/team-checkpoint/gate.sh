#!/usr/bin/env bash
# gate.sh — team-checkpoint differential gate, the registry gate_command driver.
#
# The registry (evals/oracles/registry.json) resolves this domain to:
#     evals/oracles/team-checkpoint/gate.sh --differential --seeds 200
# (bin/oracle-select team-checkpoint prints exactly that). The LIVE seeded arm: for each of N
# seeds the INDEPENDENT reference generator (reference.generate_stream) produces a deterministic
# per-HAID checkpoint stream; the SAME stream is folded by BOTH the impl (checkpoint_share) and
# the independent reference; both are normalized to served/excluded_security and DIFFED. PASS iff
# impl == reference on every seed.
#
...
usage() {
  cat <<'EOF'
team-checkpoint gate.sh — seeded impl-vs-reference differential arm
...
Usage:
  ...
  --differential   Run the seeded impl-vs-reference roster differential (the only mode). For
                   each seed: generate one per-HAID checkpoint stream, fold it through the impl
                   AND the independent reference, normalize to served/excluded_security, diff.
  --seeds N        Seeds to sweep (default 200; the registered floor).
  --start S        First seed (default 1).
  --records R      Records per generated stream (default 24).
