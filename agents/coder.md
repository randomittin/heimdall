...
- **PreToolUse Edit/Write → `heimdall-precheck-edit`** — also fires `heimdall-wip-commit note` on every real edit (best-effort, swallowed): lands a `wip:`-marked checkpoint every `HEIMDALL_WIP_EDIT_THRESHOLD` edits (default 6), independent of the PostToolUse hook's 5-distinct-file gate above. This is what makes truncation mid-task survivable even on a narrow, few-file task.
