...
```markdown
- [Heimdall](https://github.com/randomittin/heimdall) by [randomittin](https://github.com/randomittin) - A Claude Code superskill-orchestrator that gates every AI-generated change behind a falsifiable external oracle — a differential / trace-diff / verdict check the implementing agent never sees, *proven able to fail* before it can pass, so a PR opens only once real proof lands. It orchestrates waves of parallel agents across a whole team's machines behind one live presence wall (teammates auto-join by GitHub repo access — no invite, no pasted secret), and captures each dev's triage — the deny and verdict cases that would otherwise stay tribal knowledge — into a shared corpus the team learns from together. Session context checkpoints and restores automatically across runs; an optional overnight pass (`/dream`) researches, fixes, and leaves a morning report without ever auto-pushing; a visual design-match loop scores a build against its Claude Design canonical; and a falsifiable self-improvement loop tunes its own routing only on a measured delta. Nothing ships unproven.  
