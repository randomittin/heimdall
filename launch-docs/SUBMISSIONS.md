...
```markdown
- [Heimdall](https://github.com/randomittin/heimdall) by [randomittin](https://github.com/randomittin) - Your AI agents can't grade their own homework anymore. Heimdall wires an external, falsifiable oracle to every plan — a check proven able to fail before it can pass — so a PR opens when the proof lands, not when the agent says it's done. It runs parallel agents across your whole team's machines behind one live presence wall; teammates auto-join by GitHub repo access with no invite and no pasted secret, and every dev's triage lands in a shared corpus the team learns from instead of dying in one person's head. Context checkpoints and restores itself across sessions. `/dream` works the codebase overnight and leaves you a morning report — it never auto-pushes. A design-match loop scores your build against its Claude Design canonical. Nothing ships unproven.  
