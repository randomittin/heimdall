...
argument-hint: [status|check|on|auto|switch|off|where]
...
## Arming it — the real sequence (there is no `config` subcommand)

`heimdall-fallback` has exactly four subcommands: `status`, `set`, `check`,
`where`. Setting *which provider* and *which key* is not one of them — those
live directly in the JSON config file, at the path `where` prints (default
`<repo>/.heimdall/fallback.json`). Verified end-to-end against a live local
OmniRoute gateway:
