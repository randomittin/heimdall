...
**Enforcement status — WARN, not deny.** The `PreToolUse` `Agent` hook prints a notice on stderr and lets the spawn through (exit 0). It briefly denied with exit 2; that was wrong on two counts. (1) The deny's second premise — that an orchestrator could not close a named agent — became false when `TaskStop` shipped (Claude Code 2.1.198+, granted to every spawning agent in `b214eb1`). (2) The matcher fires on EVERY Agent spawn in EVERY project, because heimdall loads via `--plugin-dir`, so it hard-failed named teammates in unrelated repos that use them legitimately; from the outside that reads as agents being killed. The measurement (0/43 vs 59/66) is untouched and still sets the default. **Naming an agent transfers its lifecycle to you:** no result arrives on the spawn call, so collect via `SendMessage` and close with `TaskStop`. `TaskStop` reaches ONLY agents the calling session spawned — a cross-session stop returns `No task found with ID` — so agents stranded by a dead session need `bin/heimdall-agents orphans` instead. `HEIMDALL_ALLOW_NAMED_AGENT=1` silences the notice. The hook fails SAFE, never closed: an unparseable payload warns and still proceeds, because a malformed payload must never cost a spawn. Proof: `bash test/agent-name-gate.test.sh`.
