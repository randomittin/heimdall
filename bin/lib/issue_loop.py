...
def default_fix_runner(issue, orient_result, repo):
    """The production fix slot: a REAL Heimdall task with full gates, briefed with
    the SI-1 capsule + the normalized issue body, producing a working-tree diff in
    the repo. The loop NEVER trusts what this returns as the verdict — the verdict
    comes from the deterministic gate (SI-2 evidence), per the cardinal rule.

    The real coder dispatch is `bin/agent-pool`/the coder path; here we materialize
    the fix brief the dispatch consumes and write it into the repo's runtime home so
    the task slot (and a human auditor) can read exactly what the fix was asked to
    do. Returns { briefed: bool, brief_path: str } — descriptive only, NOT a
    verdict. Tests substitute a fixture runner; production wires the coder here."""
    home = os.environ.get("HEIMDALL_HOME") or os.path.join(repo, ".heimdall")
    ...
    return {"briefed": True, "brief_path": brief_path}
