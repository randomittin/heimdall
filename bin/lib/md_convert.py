...
#
# This module never shells out and never opens a network socket. It is a pure
# function of (path, allowed_root) → markdown, with every guard applied first.
...
def _build_converter(MarkItDown):
    ...
    installed version exposes the toggle. Defence in depth: even though we only
    ever call convert_local(), we disable plugins (arbitrary third-party
    converters) so an installed plugin cannot widen the I/O surface."""
    try:
        return MarkItDown(enable_plugins=False)
    except TypeError:
        # Older signature without the kwarg — convert_local is still local-only.
        return MarkItDown()


def convert_file(raw_path, allowed_root=None):
    """Convert a LOCAL file to markdown. Applies the full guard chain
    (assert_not_remote → sanitize_path), then calls markitdown's narrowest local
    ...
    Raises:
      ...
      UnsupportedInput     — not a regular local file / unconvertible
    ...
    try:
        if hasattr(md, "convert_local"):
            ...
            # bytes to convert_stream so dispatch is still on controlled local
            # data, never on a URI.
            with open(safe_path, "rb") as fh:
                result = md.convert_stream(fh, file_extension=os.path.splitext(safe_path)[1])
    except MarkItDownUnavailable:
        raise
...
#   4  refused — remote/URI or unsafe path (a SECURITY gate, hard fail)
...
# (which must not abort a task run) from a real security refusal (which must).
