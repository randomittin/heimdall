...
def enroll(haid, pubkey, *, provided_token, handle=None, home=None):
    ...
    The gate order (each a hard stop, fail-closed first):
      1. enroll_disabled  — the SERVER verifier secret is unset. We refuse EVERY enroll and
         NEVER allow-all. The reason names the cloud profile so a deployed instance refused for
         a missing secret is distinguishable from a bad token.
      ...
      5. haid_pubkey_conflict — haid is already registered to a DIFFERENT pubkey (409): a
         registered identity is never silently rebound, so a leaked token cannot hijack a dev.
      6. (idempotent) same haid + SAME pubkey -> ok without a rewrite.
    ...
    if existing is not None:
        if existing == pubkey:
            # Same key re-enroll: a harmless re-run, already bound. No rewrite.
    ...
    #    cp_auth.ensure_server_identity); a self-enroll never grants gate-override authority.
...
# bad request body is 422, a missing server secret is 403 (the route exists but is disabled),
