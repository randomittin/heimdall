...
# right-pinned `𝘅 NN%` (green→amber→red toward the cap) + a compact resets-in
...
#   2. ABSENT (no rate_limits) → no 𝘅 indicator anywhere (no fabrication).
...
printf '%s' "$OUT" | grep -qF '𝘅 42%' && ok "5h indicator 𝘅 42% present" || bad "5h indicator missing when rate_limits present"
# it must be the FIVE-hour value (42), NOT the seven-day (88)
printf '%s' "$OUT" | grep -qF '𝘅 88%' && bad "rendered the seven_day %, not five_hour" || ok "uses five_hour %, not seven_day"

echo "== 2) rightmost end of the top row =="
TOP="$(printf '%s' "$OUT" | head -1)"
# after stripping ANSI, the 𝘅 token sits in the final third of the top row.
...
sys.exit(0 if i!=-1 and i > len(s)*2//3 else 1)' \
  && ok "𝘅 indicator sits at the rightmost end of the top row" \
  || bad "𝘅 indicator not right-aligned"

echo "== 3) absent → nothing (no fabrication) =="
OUT0="$(render '')"
printf '%s' "$OUT0" | grep -qF '𝘅' && bad "fabricated a usage indicator with no rate_limits" || ok "no 𝘅 indicator when rate_limits absent"
...
printf '%s' "$G"  | grep -qE '38;2;34;197;94m𝘅 42%'   && ok "42% → green"  || bad "42% not green"
printf '%s' "$A_" | grep -qE '38;2;245;158;11m𝘅 75%'  && ok "75% → amber"  || bad "75% not amber"
printf '%s' "$R"  | grep -qE '38;2;239;68;68m𝘅 95%'   && ok "95% → red"    || bad "95% not red"
