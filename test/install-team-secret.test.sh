...
# Run install.sh from this working tree against a fresh HOME, with a fake claude on
# PATH and the real environment's secrets unset. CWD is set to $1 (a fresh git repo)
# so the per-repo team store resolves there — never the developer's real tree.
# stdin is /dev/null so the TTY-gated prompt path is NOT taken (mirrors curl|bash).
...
# ── (7) HERMETIC — the install runs left the repo's tracked bin/ pristine ───────
