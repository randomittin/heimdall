...
**hmd's default module set includes a proxy that can read your prompts — and hmd routes nothing through it.** [Headroom](#headroom--the-one-shipped-module-and-its-honest-limits) is `default_included: true` in [`modules/headroom/manifest.json`](modules/headroom/manifest.json): a **local** context-compression proxy which, once traffic is pointed at it, sits between your coding tool and the model provider, reads the prompts and context on their way out, and rewrites them to be smaller. Pointing traffic at it is a decision you make, not one hmd makes for you — `hmd modules add headroom` installs the package, then measures each wire the manifest declares and records what it found, and today both measure `RECORDED, not routed`. It runs as a process you own and can inspect, and it introduces no Heimdall-operated destination — that traffic goes to the same provider it went to before. Five things to know before you install:
...
Pinned to the `v2.3.8` tag **and** checked against the sha256 of that tag's `install.sh`. <!-- HEIMDALL:PIN:TAG --> The `&&`
