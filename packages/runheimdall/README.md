...
<!-- HEIMDALL:HERO-ASSET — the wall recording goes HERE, directly above the badges.
     Expected file: launch-docs/assets/wall.gif, produced by `bin/heimdall-reel record`
     (see launch-docs/assets/README.md § "The one command to get the TRUE animated wall.gif").
     Deliberately not embedded yet: that file does not exist in the repo. wall.png / wall.svg
     are static renders, not the recording — embedding either here would sell a still as the
     live wall. Once wall.gif lands, replace this comment with:
     ![Heimdall's proven wall](launch-docs/assets/wall.gif) -->
...
<!-- HEIMDALL:PIN:TAG,SHA256:BEGIN — rendered from plugin.json by bin/heimdall-render-version -->
```bash
...
Pinned to the `v2.3.8` tag **and** checked against the sha256 of that tag's `install.sh`. The `&&` <!-- HEIMDALL:PIN:TAG -->
...
<!-- HEIMDALL:PIN:TAG:BEGIN — rendered from plugin.json by bin/heimdall-render-version -->
```bash
